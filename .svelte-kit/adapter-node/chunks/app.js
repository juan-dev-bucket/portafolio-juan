...
function computeExactDuration(from = new Date(), to = new Date()) {
...
const MainTitle = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<h1${add_attribute("class", `font-[var(--title-f)] font-black tracking-[4px] text-center text-2.5em sm:text-[2em] md:text-[2.5em] lg:text-[3em] ${classes}`, 0)} style="background: linear-gradient(var(--main-text), var(--accent-text-hover)); -webkit-background-clip: text; background-clip: text;">${slots.default ? slots.default({}) : ``}</h1>`;
});
const titleSuffix = "Perfil Juan Fernando";
export {
  MainTitle as M,
  getTimeDiff as a,
  countMonths as b,
